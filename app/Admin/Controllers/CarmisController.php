...
class CarmisController extends AdminController
{
    ...
    protected function grid()
    {
        return Grid::make(new Carmis(['goods', 'sku']), function (Grid $grid) {
            ...
            $grid->filter(function (Grid\Filter $filter) {
                ...
                $filter->equal('sku_id', '规格')->select($this->skuOptions());
    ...
    {
        return Show::make($id, new Carmis(['goods', 'sku']), function (Show $show) {
            $show->field('id');
            $show->field('goods.gd_name', admin_trans('carmis.fields.goods_id'));
            $show->field('sku.sku_name', '规格');
    ...
    protected function form()
    {
        return Form::make(new Carmis(), function (Form $form) {
            ...
            $form->select('sku_id', '规格')->options($this->skuOptions())->required();
            $form->radio('status')
            ...
            $form->saving(function (Form $form) {
                if ($form->sku_id) {
                    $sku = GoodsSku::query()->find($form->sku_id);
                    if ($sku) {
                        $form->goods_id = $sku->goods_id;
                    }
                }
            });
        });
