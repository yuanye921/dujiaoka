...
class OrderService
{
    ...
    public function __construct()
    {
        ...
        $this->goodsSkuService = app('Service\GoodsSkuService');
    ...
    public function validatorGoodsSku(Request $request, Goods $goods): GoodsSku
    {
        $sku = $this->goodsSkuService->resolveForGoods($goods, $request->input('sku_id'));
        $stock = $this->goodsSkuService->availableStock($goods, $sku);
        if ($request->input('by_amount') > $stock) {
     ...
     */
    public function validatorLoopCarmis(Request $request, ?GoodsSku $sku = null)
    {
        $query = Carmis::query()
