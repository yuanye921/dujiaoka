...
class OrderProcessService
{
    ...
    public function setSku(GoodsSku $sku)
    {
        $this->sku = $sku;
    }
    ...
    {
        $price = $this->sku ? $this->sku->actual_price : $this->goods->actual_price;
    ...
    public function createOrder(): Order
    {
        try {
            ...
            $order->sku_id = $this->sku ? $this->sku->id : null;
            // 标题
            $order->title = $this->goods->gd_name . ' x ' . $this->buyAmount;
            if ($this->sku && $this->sku->sku_code !== GoodsSku::DEFAULT_SKU_CODE) {
                $order->title = $this->goods->gd_name . ' - ' . $this->sku->sku_name . ' x ' . $this->buyAmount;
            }
            // 订单类型
            ...
            // 商品单价
            $order->goods_price = $this->sku ? $this->sku->actual_price : $this->goods->actual_price;
            // 购买数量
    ...
    public function processManual(Order $order)
    {
        ...
        // 商品库存减去
        if ($order->sku_id) {
            GoodsSku::query()->where('id', $order->sku_id)->decrement('in_stock', $order->buy_amount);
        } else {
    ...
    {
        // 获得卡密
        $carmis = $this->carmisService->withGoodsByAmountAndStatusUnsold($order->goods_id, $order->buy_amount, $order->sku_id);
        // 实际可使用的库存已经少于购买数量了
